...
describe.each(editions)('$description', fs => {
    describe('listen to document updates', () => {
        ...
        test.concurrent('a lot of docs: create, listen, update, stop', async () => {
            ...
            await refs.reduce((batch, ref, id) => batch.create(ref, writeData({ id })), fs.firestore.batch()).commit();
        ...
        test.concurrent('receiving a single update on multiple updates in single txn', async () => {
            ...
            await fs.firestore.runTransaction(async txn => {
                ...
                txn.create(doc, writeData({ created: FieldValue.serverTimestamp() }));
                txn.update(doc, { updated: FieldValue.serverTimestamp() });
                txn.update(doc, { counter: FieldValue.increment(1) });
                txn.update(doc, { counter: FieldValue.increment(1) });
                txn.update(doc, { array: FieldValue.arrayUnion({ id: 1 }, { id: 2 }) });
                txn.update(doc, { array: FieldValue.arrayUnion({ id: 2 }, { id: 3 }) });
                txn.update(doc, { array: FieldValue.arrayRemove({ id: 1 }) });
            });

            const snap = readData((await nextSnap).data());
            expect(snap).toEqual({
...
function listen(doc: DocumentReference) {
    const snapshot$ = fromEventPattern<DocumentSnapshot>(value$ =>
        doc.onSnapshot(
...
function checkSnap(snap: DocumentSnapshot, expected: { createTime: Timestamp; updateTime: Timestamp; data: DocumentData }) {
    expect(readData(snap.data())).toEqual(expected.data);
