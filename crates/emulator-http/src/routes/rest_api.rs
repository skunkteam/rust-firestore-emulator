...
mod tests {
    ...
    async fn test_list_collection_ids() -> Result<()> {
        ...
        database
            .get_doc_meta_mut_no_txn(
                &"projects/demo-project/databases/(default)/documents/test_collection/test_doc"
                    .parse()?,
            )
            .await?
            .maybe_add_version(HashMap::default(), Timestamp::now())
            .await
            .expect("no version existed before, so we cannot get an Err here");
        ...
        expect_that!(result, eq(&json!({ "collectionIds": ["test_collection"] })));
