...
///     Router,
/// };
/// use hybrid_axum_tonic::GrpcStatus;
/// use hyper::Request;
///
/// async fn tonic_middleware<B>(req: Request<B>, next: Next<B>) -> Result<Response, GrpcStatus> {
///     if is_auth(&req) {
///         Ok(next.run(req).await)
///     } else {
///         Err(tonic::Status::permission_denied("Not authenticated").into())
///     }
/// }
///
...
/// }
///
/// let router: Router<()> = Router::new().layer(from_fn(tonic_middleware));
/// ```
#[derive(Debug)]
